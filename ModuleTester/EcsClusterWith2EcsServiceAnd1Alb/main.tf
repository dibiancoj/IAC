...
//add the alb listener (can only have 1 per port)
resource "aws_alb_listener" "my_alb_port_listner" {
  load_balancer_arn = module.my_alb.raw_alb.arn
  port              = local.port_for_listner
  protocol          = "HTTP"

  default_action {
    target_group_arn = module.my_ecs_service_one.raw_alb_target_group.arn
    type             = "forward"
  }
}

resource "aws_lb_listener_rule" "for_api_one" {
  listener_arn = aws_alb_listener.my_alb_port_listner.arn
  priority     = 100
  ...
  condition {
    path_pattern {
      values = ["/api1"] //this is the url in the alb. So /lambdaName/ after the alb dns name will execute your lambda (make you put the trailing slash)
    }
  }
}

resource "aws_lb_listener_rule" "for_api_two" {
  listener_arn = aws_alb_listener.my_alb_port_listner.arn
  priority     = 200

  action {
    type             = "forward"
    target_group_arn = module.my_ecs_service_two.raw_alb_target_group.arn
  }

  condition {
    path_pattern {
      values = ["/api2"] //this is the url in the alb. So /lambdaName/ after the alb dns name will execute your lambda (make you put the trailing slash)
    }
  }
}
