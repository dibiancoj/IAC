locals {
  vpc_id                   = "vpc-5dd17e36"
  lambda_zip_file_location = "./artifacts/myfunction.zip"
}
...
//create the bucket which is completely private
resource "aws_s3_bucket" "my_bucket_for_lambda_code" {
  bucket = "my-tf-test-bucket-abcdef-test"
}

resource "aws_s3_bucket_acl" "my_bucket_for_lambda_code_policy" {
  bucket = aws_s3_bucket.my_bucket_for_lambda_code.id
  acl    = "private"
}
...
//now push your lambda zip file / code to the s3 bucket
resource "aws_s3_object" "object" {
  bucket = aws_s3_bucket.my_bucket_for_lambda_code.id
  key    = "my-lambda"
  source = local.lambda_zip_file_location
  etag   = filemd5(local.lambda_zip_file_location)
}
...
module "lambda_that_gets_triggered_one" {
  ...
  subnet_ids                    = []
  lambda_zip_file_name          = local.lambda_zip_file_location
  lambda_s3_key                 = aws_s3_object.object.key
  lambda_s3_bucket              = aws_s3_object.object.bucket
